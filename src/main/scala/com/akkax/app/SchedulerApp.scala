...
object SchedulerApp extends App {
  ...
  val processor: ActorRef = actorSystem.actorOf(Props(new ProcessorActor), "processor")
  actorSystem.scheduler.scheduleWithFixedDelay(0 milliseconds, 1000 milliseconds, processor, Create(" log1 ", " File Content  1 "))
  val request = List(
